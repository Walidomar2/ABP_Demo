...
* Check the `ConnectionStrings` in `appsettings.json` files under the `ABP.Demo.HttpApi.Host` and `ABP.Demo.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p dd325aea-5ce4-43a8-9100-bd93712fdf7a
```

> `dd325aea-5ce4-43a8-9100-bd93712fdf7a` is the password of the certificate, you can change it to any password you want.
...
Run `ABP.Demo.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `ABP.Demo.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `ABP.Demo.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
